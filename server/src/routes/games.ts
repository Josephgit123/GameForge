...
gamesRouter.get('/mine/all', requireAuth, requireRole(Role.PUBLISHER), async (req, res) => {
...
gamesRouter.post('/', requireAuth, requireRole(Role.PUBLISHER), async (req, res) => {
  const publisher = await prisma.publisher.findUnique({ where: { userId: req.user!.id } });
  if (!publisher) {
    return res.status(403).json({ error: 'You have not registered as a publisher yet' });
  }
...
gamesRouter.patch('/:id', requireAuth, requireRole(Role.PUBLISHER, Role.ADMIN), async (req, res) => {
  const game = await prisma.game.findUnique({ where: { id: req.params.id } });
  if (!game) {
    return res.status(404).json({ error: 'Game not found' });
  }
  ...
  if (status !== undefined) {
    const allowedForRole =
      req.user!.role === Role.ADMIN
        ...
        : [GameStatus.DRAFT, GameStatus.PUBLISHED];
